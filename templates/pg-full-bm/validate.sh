#!/bin/sh
# pg-full-bm validate — all checks inside Docker
set -e
...
PSQL_P() { docker exec -e PGPASSWORD="$PG_PASSWORD" pg-full-bm-postgres-1 psql -U "$PG_USER" -h localhost -d "$PG_DATABASE" "$@"; }
PSQL_D() { docker exec -e PGPASSWORD="$PG_PASSWORD" pg-full-bm-postgres-1 psql -U "$PG_USER" -h pgdog -p 6432 -d "$PG_DATABASE" "$@"; }

echo "=== pg-full-bm validate ==="

echo -n "Primary: "
docker exec -e PGPASSWORD="$PG_PASSWORD" "$CONTAINER" pg_isready -U "$PG_USER" -h localhost -q && echo "OK" || echo "FAIL"

echo -n "Replica: "
docker exec -e PGPASSWORD="$PG_PASSWORD" pg-full-bm-pg-replica-1 pg_isready -U "$PG_USER" -h localhost -q && echo "OK" || echo "FAIL"

echo -n "PgDog: "
docker exec -e PGPASSWORD="$PG_PASSWORD" pg-full-bm-postgres-1 psql -U "$PG_USER" -h pgdog -p 6432 -d "$PG_DATABASE" -tAc "SELECT 1" 2>/dev/null | grep -q "1" && echo "OK" || echo "FAIL"
...
echo -n "Replica lag: "
LAG=$(docker exec -e PGPASSWORD="$PG_PASSWORD" pg-full-bm-pg-replica-1 psql -U "$PG_USER" -h localhost -d "$PG_DATABASE" -tAc \
