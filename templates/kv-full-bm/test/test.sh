#!/bin/sh
# kv-full-bm integration test — full PITR cycle (backup -> disaster -> restore -> verify)
set -e
...
echo "=== kv-full-bm INTEGRATION TEST ==="
...
docker run --rm -v "kv-full-bm_primary_data:/data" -v "$BACKUP_FILE:/backup:ro" \
...
echo ""
echo "=== kv-full-bm INTEGRATION TEST PASSED ==="
