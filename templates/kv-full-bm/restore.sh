#!/bin/sh
# kv-full-bm restore — restore Dragonfly from .dfs snapshot
...
if [ -d "$SNAPSHOT" ]; then
  ...
  docker run --rm -v "kv-full-bm_primary_data:/data" -v "$SRC_DIR:/backup:ro" \
    alpine sh -c "cp /backup/*.dfs /data/ && chmod 644 /data/*.dfs" 2>/dev/null
  docker run --rm -v "kv-full-bm_replica_data:/data" -v "$SRC_DIR:/backup:ro" \
    alpine sh -c "cp /backup/*.dfs /data/ && chmod 644 /data/*.dfs" 2>/dev/null
else
  SNAPSHOT_NAME=$(basename "$SNAPSHOT")
  docker run --rm -v "kv-full-bm_primary_data:/data" -v "$(dirname "$(realpath "$SNAPSHOT")"):/backup:ro" \
    alpine sh -c "cp /backup/$SNAPSHOT_NAME /data/dump.dfs && chmod 644 /data/dump.dfs" 2>/dev/null
  docker run --rm -v "kv-full-bm_replica_data:/data" -v "$(dirname "$(realpath "$SNAPSHOT")"):/backup:ro" \
    alpine sh -c "cp /backup/$SNAPSHOT_NAME /data/dump.dfs && chmod 644 /data/dump.dfs" 2>/dev/null
fi
echo "  Volumes ready"
