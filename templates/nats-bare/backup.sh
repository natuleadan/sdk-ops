#!/bin/bash
# nats-bare backup - seal JetStream streams to S3 with a NATS NKey (curve).
# Port of the nats-dockerized backup (native paths, no Docker). Runs on the
# node (systemd timer or cron). Reads its secrets from the service .env.
