#!/bin/bash
# nats-bare integration test - exercises the node like an operator (native
# nats-server + systemd, no Docker):
#   Step 0  env + binaries (nats CLI, app account, systemd unit)
...
#   Step 5  failover simulation (stop/start the local systemd unit)
#   Step 6  S3 DR cycle (seal -> S3 -> delete -> unseal -> restore, s3cmd)
...
# [SKIP]ped (the recipient NKey never leaves the operator's machine). The S3
# calls run through a temp s3cmd config built from those env vars (no Docker,
# no mc client).
#
...
if ! command -v systemctl >/dev/null 2>&1; then
  echo "  [FAIL] systemctl not found (nats-bare runs as a systemd unit)"
  echo "=== nats test FAILED ==="
  exit 1
fi
if systemctl is-active --quiet nats-server 2>/dev/null; then
  echo "  [OK] systemd unit nats-server active"
else
...
# Unit name comes from the rendered nats.conf install (init.sh). Read it
# defensively: fall back to nats-server if the unit file is not there.
UNIT_NAME="nats-server"
if [ -f /etc/systemd/system/nats-server.service ]; then
  echo "  [OK] systemd unit file: /etc/systemd/system/nats-server.service"
else
  echo "  [WARN] unit file missing at /etc/systemd/system/nats-server.service"
fi
echo "  [OK] unit: $UNIT_NAME"
...
# Bare-metal failover: stop/start the systemd unit (the dockerized test used
# docker compose stop/start on the compose service).
if ! systemctl is-active --quiet "$UNIT_NAME" 2>/dev/null && ! systemctl list-unit-files "$UNIT_NAME.service" >/dev/null 2>&1; then
  echo "  [SKIP] unit $UNIT_NAME not installed on this host"
else
  if ! systemctl stop "$UNIT_NAME" >/dev/null 2>&1; then
    echo "  [FAIL] systemctl stop $UNIT_NAME"
    FAILED=1
  else
    echo "  [OK] local server stopped ($UNIT_NAME)"
    ...
    if ! systemctl start "$UNIT_NAME" >/dev/null 2>&1; then
      echo "  [FAIL] systemctl start $UNIT_NAME"
      FAILED=1
    else
      echo "  [OK] local server restarted ($UNIT_NAME)"
...
# upload only and document the unseal half as [SKIP] (still PASS). The S3
# client is s3cmd (installed by init.sh) with a temp config built from the
# S3_* env - no Docker/mc on a bare node.
if [ -z "${S3_BUCKET:-}" ] || [ -z "${S3_ACCESS_KEY:-}" ] || [ -z "${S3_SECRET_KEY:-}" ]; then
  ...
  echo "  [SKIP] S3 DR (s3cmd required on a bare node)"
else
  : "${S3_ENDPOINT:=s3.us-east-005.backblazeb2.com}"
  # Scheme-aware endpoint: strip an explicit scheme for s3cfg (host_base is a
  # bare host); http:// disables TLS (local/MinIO testing), https otherwise.
  S3_HOST="${S3_ENDPOINT#*://}"
  case "$S3_ENDPOINT" in
    http://*) S3_HTTPS="False" ;;
    *)        S3_HTTPS="True" ;;
  ...
  # Temp s3cmd config (same layout writeS3Cfg uses for the operator's node).
  S3CFG="$WORK/s3cfg"
  cat > "$S3CFG" <<EOF
[default]
...
EOF
  ...
  if ! "$BIN" stream add "$DR_STREAM" '--subjects=demo.dr.>' --storage=file --replicas=1 --retention=limits --max-age=1h --discard=old --defaults "${APP[@]}" >/dev/null 2>&1; then
    ...
    # key, then upload the sealed blob with s3cmd.
    if "$BIN" stream backup "$DR_STREAM" "$WORK/$DR_STREAM" --consumers "${APP[@]}" >/dev/null 2>&1 \
      ...
      if S3 put "$WORK/$DR_STREAM.nkey" "s3://$S3_BUCKET/$S3_KEY"; then
    ...
    if [ "$DR_OK" -eq 1 ] && [ -n "$RECIPIENT_NK" ] && [ -f "$RECIPIENT_NK" ]; then
      if [ -n "$SENDER_PUB" ]; then
        if S3 get "s3://$S3_BUCKET/$S3_KEY" "$WORK/backup.nkey" \
      ...
      # s3cmd ls prints the object keys on stdout.
      echo "  [SKIP] unseal (operator NKey not on this host)"
      if s3cmd -c "$S3CFG" ls "s3://$S3_BUCKET/$S3_PREFIX/$DR_STREAM/" 2>/dev/null | grep -q "$TS"; then
    ...
    if S3 rm "s3://$S3_BUCKET/$S3_KEY"; then
