...
# rps.sh - redis-benchmark RPS for df-bare (native, no Docker).
# Usage (from the VPS host, requires redis-tools from init.sh):
#   bash rps.sh                       # primary 127.0.0.1:6379
#   bash rps.sh 127.0.0.1 6443 dragonfly   # through the HAProxy TLS port needs
#                                     # TLS client flags - use the native port
...
echo "============================================"
echo " redis-benchmark RPS - df-bare"
