...
PSQL_P() { docker exec -e PGPASSWORD="$PG_PASSWORD" pgsql-docker-postgres-1 psql -U "$PG_USER" -h localhost -d "$PG_DATABASE" "$@"; }
PSQL_D() { docker exec -e PGPASSWORD="$PG_PASSWORD" pgsql-docker-postgres-1 psql -U "$PG_USER" -h pgdog -p 6432 -d "$PG_DATABASE" "$@"; }
...
echo -n "Replica lag: "
LAG=$(docker exec -e PGPASSWORD="$PG_PASSWORD" pgsql-docker-pg-replica-1 psql -U "$PG_USER" -h localhost -d "$PG_DATABASE" -tAc \
